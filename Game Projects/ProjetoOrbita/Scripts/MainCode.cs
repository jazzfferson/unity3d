...
public class MainCode : MonoBehaviour
{
		 ...
		 if (!landed)
			{
				...
				foguete.GetComponent<Rigidbody>().isKinematic = true;
				foguete.mesh.GetComponent<Renderer>().enabled = false;
	...
	{
		 foguete.GetComponent<Rigidbody>().drag = 0.1f;
		 canLand = true;	
	}
	...
	void ReInitializeLevel()
	{
		...
		GetComponent<FieldForce>().CreateField(landTarget.transform, 8,1,8,4,0.6f,1,0.1f * timeScale,0.5f* timeScale,2,2* timeScale,0,360,GoEaseType.CubicInOut);
	}
	...
	{	
		...
		{
            if (foguete.GetComponent<Rigidbody>().velocity.magnitude > 0.3 && executeAnimation)
                foguete.GetComponent<Rigidbody>().drag += Time.deltaTime * foguete.GetComponent<Rigidbody>().velocity.magnitude * 30;
            else
            {
                if (executeAnimation)
                {
    ...
    {
        distanciaPercorrida += foguete.GetComponent<Rigidbody>().velocity.magnitude * Time.deltaTime* 100000;
    }
