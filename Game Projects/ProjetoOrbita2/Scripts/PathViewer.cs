...
public class PathViewer : MonoBehaviour {
	...
	void Update () {
		...
		{
			rotation = Root.GetComponent<Rigidbody>().velocity.normalized;
		  ...
		  if(colorSpeedBased)
		  sprite.GetComponentInChildren<UISprite>().color = new Color(Root.GetComponent<Rigidbody>().velocity.magnitude/10,Root.GetComponent<Rigidbody>().velocity.magnitude/10,1,1);
